...
function passwordResetHtml(actionLink: string) {
  ...
  <div style="font-family:Manrope,Helvetica,Arial,sans-serif;background:#f3efe6;padding:32px 16px;color:#1c2a1f;">
    <div style="max-width:480px;margin:0 auto;background:#fff;border-radius:12px;padding:28px;border:1px solid rgba(31,77,58,0.12);">
      ...
      <h1 style="margin:0 0 12px;font-size:24px;color:#1f4d3a;">비밀번호 재설정</h1>
      <p style="margin:0 0 20px;line-height:1.6;color:#4d5c52;">아래 버튼을 눌러 새 비밀번호를 설정하세요. 요청하지 않았다면 이 메일을 무시해도 됩니다.</p>
      <p style="margin:0 0 24px;">
        <a href="${actionLink}" style="display:inline-block;background:#1f4d3a;color:#fff;text-decoration:none;padding:12px 18px;border-radius:8px;font-weight:600;">새 비밀번호 설정</a>
      </p>
      <p style="margin:0;font-size:12px;line-height:1.5;color:#4d5c52;word-break:break-all;">버튼이 작동하지 않으면 이 링크를 복사하세요:<br/>${actionLink}</p>
    </div>
  </div>`;
}
