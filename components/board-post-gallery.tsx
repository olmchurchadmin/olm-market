...
export function BoardPostGallery({
  title,
  images,
}: {
  title: string;
  ...
  const { t } = useI18n();
  const urls = images
    .map((img) => boardImageUrl(img.storage_path))
        ...
        {urls.map((url, index) => (
          <li key={`${url}-${index}`}>
            <button
              ...
              className="relative aspect-square w-full cursor-zoom-in overflow-hidden rounded-md border border-brand/10 bg-white"
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={url}
